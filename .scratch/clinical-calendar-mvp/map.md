...
- Tickets 76, 79, and 80 complete native reminder/notification policy, explicit synchronization conflict resolution, and passwordless identity/Connected Devices. Production notifications now reconcile repository truth with per-device privacy and synchronized snooze state; conflicts preserve both originals and converge through normal mutations; Supabase OTP sessions, revocation, email change, and exact device-local removal are live-validated. The combined repository quality gate, 25 identity plus 28 sync pgTAP assertions, schema lint, revoke concurrency, real local GoTrue/Mailpit flow, and Windows/Android debug builds pass. Ticket 81 Trash, recovery, and account erasure is the next unblocked frontier; iOS runtime verification remains deferred to tickets 87 and 88.
